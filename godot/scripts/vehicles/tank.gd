...
func get_cannon_muzzle() -> Vector3:
	...
	return global_position - global_transform.basis.z * 5.0 + Vector3.UP * 2.2
...
func _ensure_collision() -> void:
	...
	if _collision_shape == null:
		...
		_collision_shape.position = Vector3(0.0, 0.7, 0.0)
		add_child(_collision_shape)
	if _collision_shape.shape == null:
		var shape := BoxShape3D.new()
		shape.size = Vector3(3.3, 1.4, 6.2)
...
func _ensure_camera() -> void:
	...
	if _camera == null:
		...
		_camera.position = Vector3(0.0, 2.6, -0.35)
		_camera.fov = 70.0
		_camera.near = 0.05
...
func _build_model() -> void:
	...
	var olive_mat := _make_material(Color(0.33, 0.37, 0.21), 0.78, 0.2)
	var dark_green_mat := _make_material(Color(0.14, 0.18, 0.12), 0.92, 0.0)
	var dark_gray_mat := _make_material(Color(0.10, 0.10, 0.10), 0.95, 0.0)

	_add_box(_model, Vector3(3.2, 0.9, 6.0), Vector3(0.0, 0.7, 0.0), olive_mat)
	_add_box(_model, Vector3(3.2, 0.45, 1.1), Vector3(0.0, 1.2, -2.55), olive_mat, Vector3(-28.0, 0.0, 0.0))
	_add_box(_model, Vector3(3.2, 0.4, 0.9), Vector3(0.0, 1.05, 2.55), olive_mat, Vector3(32.0, 0.0, 0.0))
	_add_box(_model, Vector3(3.0, 0.5, 3.7), Vector3(0.0, 1.35, 0.15), olive_mat)
	_add_box(_model, Vector3(0.12, 0.8, 3.0), Vector3(-1.58, 1.15, -0.2), dark_green_mat)
	_add_box(_model, Vector3(0.12, 0.8, 3.0), Vector3(1.58, 1.15, -0.2), dark_green_mat)
	_add_box(_model, Vector3(0.72, 0.95, 6.3), Vector3(-1.72, 0.62, 0.0), dark_gray_mat)
	_add_box(_model, Vector3(0.72, 0.95, 6.3), Vector3(1.72, 0.62, 0.0), dark_gray_mat)

	for side in [-1.0, 1.0]:
		for wheel_z in [-2.3, -1.4, -0.5, 0.4, 1.3, 2.2]:
			var wheel := _add_road_wheel(Vector3(side * 1.8, 0.32, wheel_z), dark_gray_mat)
			_track_wheels.append(wheel)

	_turret = Node3D.new()
	_turret.name = "Turret"
	_turret.position = Vector3(0.0, 1.72, -0.2)
	_model.add_child(_turret)

	_add_cylinder(_turret, 0.55, 0.95, Vector3(0.0, 0.25, 0.0), olive_mat)
	_add_cylinder(_turret, 0.28, 0.82, Vector3(0.0, 0.6, 0.0), dark_green_mat)
	_add_box(_turret, Vector3(1.15, 0.5, 0.38), Vector3(0.0, 0.22, -0.82), olive_mat, Vector3(-8.0, 0.0, 0.0))
	_add_box(_turret, Vector3(0.6, 0.1, 0.7), Vector3(-0.32, 0.82, -0.15), dark_green_mat)
	_add_box(_turret, Vector3(0.6, 0.1, 0.7), Vector3(0.32, 0.82, -0.15), dark_green_mat)
	_add_box(_turret, Vector3(0.3, 0.18, 0.14), Vector3(-0.3, 0.88, -0.55), dark_gray_mat)
	_add_box(_turret, Vector3(0.3, 0.18, 0.14), Vector3(0.3, 0.88, -0.55), dark_gray_mat)

	_barrel = Node3D.new()
	_barrel.name = "Barrel"
	_barrel.position = Vector3(0.0, 0.52, -0.4)
	_turret.add_child(_barrel)

	_add_cylinder(_barrel, 5.1, 0.09, Vector3(0.0, 0.0, -2.55), dark_green_mat, Vector3(-90.0, 0.0, 0.0))
	_add_cylinder(_barrel, 0.35, 0.13, Vector3(0.0, 0.0, -2.0), dark_gray_mat, Vector3(-90.0, 0.0, 0.0))
	_muzzle = Node3D.new()
	_muzzle.name = "Muzzle"
	_muzzle.position = Vector3(0.0, 0.0, -4.85)
	_barrel.add_child(_muzzle)
	_add_cylinder(_muzzle, 0.45, 0.15, Vector3.ZERO, dark_gray_mat, Vector3(-90.0, 0.0, 0.0))

	_add_box(_model, Vector3(0.35, 0.22, 0.12), Vector3(-1.1, 1.05, -2.92), dark_gray_mat)
	_add_box(_model, Vector3(0.35, 0.22, 0.12), Vector3(1.1, 1.05, -2.92), dark_gray_mat)
...
func _add_road_wheel(wheel_position: Vector3, material: StandardMaterial3D) -> Node3D:
	...
	_add_cylinder(wheel, 0.46, TRACK_WHEEL_RADIUS, Vector3.ZERO, material, Vector3(90.0, 0.0, 0.0))
	return wheel
